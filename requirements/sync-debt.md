...
## Чувствительные данные в Kafka-событии VERIFY_REQUESTED — закрыто 2026-08-02

Оформлено как [ADR-008](../decisions/adr-008-pii-in-kafka-events.md): событие `VERIFY_REQUESTED`
несёт только базовые данные анкеты и ссылки на документы; паспорт/СНИЛС/ИНН/адреса KYC забирает
отдельным запросом `GET /v1/internal/kyc-data/{applicationId}` (`api/openapi.yaml`, схема
`KycServiceAuth`) — читает уже существующую `Applicant_sensitive_data`, нового хранилища не
потребовалось. Диаграмма happy path обновлена. Открыт только механизм самой аутентификации
KYC (mTLS/сервисный ключ — конкретика не выбрана) — см. ADR-008 "Открытые вопросы".
